local src_dir = "$(projectdir)/src/em_log"

target("test-log")
    set_kind("binary")
    add_includedirs(src_dir, ".")
    add_files(path.join(src_dir, "log.c"), "test_log.c")
    add_files("$(projectdir)/src/em_dstream/ring_buffer.c")
    add_files("$(projectdir)/src/em_platform/async.c")
    add_files("$(projectdir)/src/em_platform/time_util.c")
    add_defines("USE_CUSTOM_CPU_ADAPTER=1")
    if is_os("windows") then
        add_files("$(projectdir)/src/em_platform/adapters/win32/cpu_adapter.c")
    end
    if is_os("linux") then
        add_files("$(projectdir)/src/em_platform/adapters/linux/cpu_adapter.c")
    end
...
target("test-simple_logger-fast")
    ...
    add_includedirs(src_dir, ".")
    add_files(path.join(src_dir, "simple_logger.c"), "test_simple_logger.c")
    add_files("$(projectdir)/src/em_format/format.c")
