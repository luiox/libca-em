...
```c
#include <em_driver/ir_track/ir_track.h>
```
...
em.add_libs(target, "em_driver", {
    ir_track = {
        mode = "extern",
        port = {path.join(os.scriptdir(), "board", "port_ir_track.c")}
    }
})
```
...
- 清单：`src/em_driver/ir_track/ir_track.lua`
- 源码目录：`src/em_driver/ir_track/`
