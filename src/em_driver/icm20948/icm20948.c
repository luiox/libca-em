...
{
	pIICInterface_t iicdev = &User_sIICDev;//初始化iic设备
...
{
	pIICInterface_t iicdev = &User_sIICDev;//指定iic设备
...
static void attitudeUpdate(IMU_DATA_t imudata, ATTITUDE_DATA_t *attitude) // 显示互补滤波（陀螺仪 + 加速度计）
{
	...
	float halftime = 0.005f * 0.5f; // 5ms的采样周期
	...
	//弧度转化为度
	attitude -> roll*= 57.2957f;
	attitude -> pitch*= 57.2957f;
	attitude -> yaw*= 57.2957f;
