...
// CRC-16/MODBUS 查找表 (高位)
// 多项式: 0x8005, x^16 + x^15 + x^2 + 1
static const u8 g_crc16_modbus_table_high[] = {
...
// CRC-16/MODBUS 查找表 (低位)
static const u8 g_crc16_modbus_table_low[] = {
...
u16 crc16_xmodem_fast(const void* data, usize size)
{
    ...
    const u8* p = (const u8*)data;
    for (usize i = 0; i < size; i++) {
        crc = (crc << 8) ^ g_crc16_ccitt_table[((crc >> 8) ^ (u16)p[i]) & 0xFF];
    }
    return crc;
}
...
{
    return crc16_xmodem_fast(data, size);
}
